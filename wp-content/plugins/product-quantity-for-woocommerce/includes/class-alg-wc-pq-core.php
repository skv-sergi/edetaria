...
class Alg_WC_PQ_Core {
	...
	function validate_on_add_to_cart( $passed, $product_id, $quantity, $variation_id = 0 ) {
		...
		if ($variation_id > 0)
		{
			foreach ( array( 'allowed', 'disallowed' ) as $allowed_or_disallowed ) {
				if ( 'yes' === get_option( 'alg_wc_pq_exact_qty_' . $allowed_or_disallowed . '_section_enabled', 'no' ) ) {
					if ( ! $this->check_product_exact_qty( $variation_id, $allowed_or_disallowed, $quantity, $cart_item_quantity ) ) {
						$this->messenger->print_message( 'exact_qty_' . $allowed_or_disallowed, false, $this->get_product_exact_qty( $variation_id, $allowed_or_disallowed ), $quantity, $variation_id );
						return false;
					}
				}
			}
	...
	function set_quantity_input_args( $args, $product ) {
		...
		if ( 'disabled' != ( $force_on_single = get_option( 'alg_wc_pq_force_on_single', 'disabled' ) ) && is_product() ) {
			...
			if('default' === ( $force_on_single = get_option( 'alg_wc_pq_force_on_single', 'disabled' ))) {
				$args['input_value'] = $this->get_product_qty_default( $product->id, $args['input_value'] );
			} else {
	...
	function ajax_price_by_qty( $param ) {
		if ( isset( $_POST['alg_wc_pq_qty'] ) && '' !== $_POST['alg_wc_pq_qty'] && ! empty( $_POST['alg_wc_pq_id'] ) ) {
			...
			$unit = get_post_meta($product_id, '_alg_wc_pq_qty_price_by_qty_unit_label_template_singular', true);
			$units = get_post_meta($product_id, '_alg_wc_pq_qty_price_by_qty_unit_label_template_plural', true);
			$unit = ( ( !empty($unit) && 'yes' === get_option( 'alg_wc_pq_qty_price_by_qty_unit_input_enabled', 'no' ) ) ? $unit : 'pcs' );
			$units = ( ( !empty($units)&& 'yes' === get_option( 'alg_wc_pq_qty_price_by_qty_unit_input_enabled', 'no' ) ) ? $units : 'pcs' );
	...
	function get_quantity_with_sep( $qty ){
		if ('yes' === apply_filters( 'alg_wc_pq_qty_dropdown_thousand_separator', 'no') ) {
