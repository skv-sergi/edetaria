 ...
 *
 * An instance of this class will group elements from the cart to posterior comparation on selection rules.
...
class Fish_n_Ships_group {
	...
	public $elements_unmatched   = array();  // but when another this or another group is unmatched, coincident elements will be moved here
		...
		foreach ($this->elements as $product) {
			...
			switch ($what) {
				...
				case 'by-price':
					...
					$item_value = $product[ 'data' ]->get_price();
				...
				case 'by-volume':
					...
					$value      = $dimensions[0] * $dimensions[1] * $dimensions[2];
					$item_value = $dimensions[0].'x'.$dimensions[1].'x'.$dimensions[2].'='.$value; // just for log
			...
			if ($this->group_by === 'none') {
				...
				$this->shipping_class->debug_log('. Ungrouped, calculate: #' . $product['data']->get_id() . ' ' . $Fish_n_Ships->get_name($product) . ' (' . $item_value . ' * ' . $qty . ($forced_to_1 ? ' [forced]' : '') . ') accomulated: ' . $value, 3 );
			
			} else {
			
				$this->shipping_class->debug_log('+ #' . $product['data']->get_id() . ' ' . $Fish_n_Ships->get_name($product) . ' (' . $item_value . ' * ' . $qty . ') accomulated: ' . $value, 4 );
			}
		}
